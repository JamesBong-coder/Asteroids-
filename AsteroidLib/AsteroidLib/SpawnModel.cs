...
namespace AsteroidLib
{
    ...
    public class SpawnModel
    {
        ...
        public bool SpawnAsteroids(PointF pos)
        {
            for (int i = 0; i < 5; i++)
            {
                asteroids.Add(new AsteroidModel(SpeedObjects * 2, new MoveBase(pos, rnd.Next(0, 360)), false, SizeAster));
            }
            return true;
        }
            ...
            for (int i = 0; i < asteroids.Count; i++)
            {
                if (asteroids[i].CheckAster())
                {
                    if (asteroids[i].isBig)
                        SpawnAsteroids(asteroids[i].Pos);
